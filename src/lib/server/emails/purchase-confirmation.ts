...
export const buildPurchaseConfirmationEmail = ({
	...
	const body = `
		<p style="margin: 24px 0 0; color: #231722; font-size: 16px; line-height: 1.8;">Hola ${escapeHtml(buyerName)}, recibimos tu compra y ya registramos tu pedido ${escapeHtml(orderNumber)} en ZonaPantys.</p>
		${renderOrderItemsTable(items)}
		<div style="margin-top: 18px; text-align: right; color: #231722; font-size: 18px; font-weight: 800;">Total: ${formatCurrency(total)}</div>
