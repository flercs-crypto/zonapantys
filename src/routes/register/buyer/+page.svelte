<script lang="ts">
	...
	const seo = $derived.by<SeoMetadata>(() => {
		$currentLocale;
		return {
			title: m.auth_register_buyer_page_title(),
			description: m.auth_register_buyer_page_description(),
...
<main class="flex min-h-screen items-center justify-center px-4 py-10" data-locale={$currentLocale}>
	<div class="w-full max-w-md space-y-8">
		<AuthBrandHeader title="Zonapantys" subtitle={m.auth_register_buyer_copy()} />
		<RegisterForm
			role="buyer"
			{mode}
