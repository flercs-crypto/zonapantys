<script lang="ts">
	...
	const seo = $derived.by<SeoMetadata>(() => {
		$currentLocale;
		return {
			title: m.auth_register_page_title(),
			description: m.auth_register_page_description(),
	...
	<div class="w-full max-w-md space-y-8">
		...
		<section class="rounded-[1.5rem] border border-slate-200 bg-white p-8 shadow-card">
			...
			<footer class="mt-6 text-center text-sm text-slate-600">
				...
				<a class="font-semibold text-brand hover:text-brand-dark" href={resolve('/login')}
					>{m.common_login()}</a
				>
			</footer>
		</section>
